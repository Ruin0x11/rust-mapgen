use maker::{MakerError, MakerResult};
use rand::{self, Rng};
use hlua::{self, Lua};

fn lua_between(a: i32, b: i32) -> MakerResult<i32> {
    if a >= b {
        return Err(MakerError::BadRange(a, b))
    }
    Ok(rand::thread_rng().gen_range(a, b))
}

fn lua_zero_to(n: i32) -> MakerResult<i32> {
    if n <= 0 {
        return Err(MakerError::BadRange(0, n))
    }
