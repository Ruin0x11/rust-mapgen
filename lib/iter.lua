  ...
  repeat
    if self.cur_x >= self.max_x then
      ...
      if self.cur_y > self.max_y then
        ...
        point = self:check_filter(wrl.point(self.cur_x, self.cur_y))
        if point == nil then
          point = -2
        end
      end
...
function iter.rect_size_iterator(top_corner, size, filter, rvi)
  return iter.rect_iterator(top_corner, top_corner + size - wrl.point(1, 1),
                            filter, rvi)
end
...
function iter.map_iterator(filter, rvi)
   return iter.rect_iterator(wrl.point(0, 0), wrl.size(), filter, rvi)
end
...
function iter.circle_iterator (radius, ic, filter, center, rvi)
  ...
  local top_corner = wrl.point(y_x - radius, y_y - radius)
  local bottom_corner = wrl.point(y_x + radius, y_y + radius)

  local function check_dist (point)
    local _x, _y = point:xy()
    local dist = wrl.distance(y_x, y_y, _x, _y)
    local npoint = nil
...
function iter.adjacent_iterator (ic, filter, center, rvi)
  local y_x, y_y = wrl.point(0, 0)

  if center ~= nil then
    y_x, y_y = center:xy()
  end

  local top_corner = wrl.point(y_x - 1, y_y - 1)
  local bottom_corner = wrl.point(y_x + 1, y_y + 1)
  local include_center = ic
